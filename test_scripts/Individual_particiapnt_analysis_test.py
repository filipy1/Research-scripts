...
### An app to zoom-in into the data of an indiviudal participant in the research.



st.set_page_config(
    page_title="Indiviudal participant analysis",
)
...
| First Index   | Group         | Question 1    | Question 2    | 
| ------------- | ------------- | ------------- | ------------- | 
| USER ID       | Active/Placebo| Numerical ans | Numerical ans |
| USER ID       | Active/Placebo| Numerical ans | Numerical ans |
| USER ID       | Active/Placebo| Numerical ans | Numerical ans |
| USER ID       | Active/Placebo| Numerical ans | Numerical ans |
\n \n """)
...
uploaded_first_baseline = file_col1.file_uploader("Choose a baseline data CSV/XLSX file")

uploaded_file = file_col2.file_uploader("Choose a DB CSV/XLSX file")
...
df["Date"] = pd.to_datetime(df["Date"], format="%d/%m/%Y") ### Weeks are calculated from sunday to saturday (Jewish week).
...
def pain_vas_graph(df, user_n="1"):
    ...
    df.drop(["Week", "Day_of_week"], axis=1, inplace=True) # Removing columns we made earlier
    category = st.radio("Choose column to display", df.columns[1:])
    ...
    ax.set_title(f"{category} by date, user {user_n}")
    st.pyplot(fig)
