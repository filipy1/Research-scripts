...
st.set_page_config(
    page_title="Comparison of participant's answers",
)

st.write("# Participant's comprasion app")

st.markdown(
    """ An app that shows interactive graphs to help examine the behaviour of participant's compared to each other.
    \n You'll need the basic DB file and the group* file for this script.
    \n \n
    * Group file format:
| First Index   | Group         |
| ------------- | ------------- |
| USER ID       | Active/Placebo|
| USER ID       | Active/Placebo|
| USER ID       | Active/Placebo|
| USER ID       | Active/Placebo|
\n \n """)
...
)  # Uploading a CSV/XLSX file as a data-base
