...
def bar_graph_data(df, user_n="1", base_line=None):
    ...
    users_dfs = create_indiviudal_user_dfs(df) # Dictionary of DF's for every user in the CSV
    try:
    ...
    grouped_week = user.groupby("Week").sum()  # Preping dataframe for weekly answers visualization 
    ...
    all = col2.checkbox('all')   # Creating checkboxes for every column in our DF and an option to select 'all'.
    if all:
